...
const ATSScoreCard: React.FC<ATSScoreCardProps> = ({ resumeFile, jobDescFile }) => {
  // Placeholder for actual logic later
  const score = 75;
    ...
    <div className="bg-white p-6 rounded-xl shadow-lg hover:shadow-xl transition-shadow">
      ...
      <ul className="space-y-3">
        <li className="p-4 bg-slate-50 rounded-lg hover:bg-slate-100 transition-all">
          ...
          <p className="text-slate-600 text-sm">Add specific keywords from the job description to your skills section.</p>
        </li>
        <li className="p-4 bg-slate-50 rounded-lg hover:bg-slate-100 transition-all">
          <p className="font-semibold text-blue-600">Quantifiable Results</p>
          <p className="text-slate-600 text-sm">Use numbers to show your impact (e.g. “Improved X by 25%”).</p>
        </li>
        <li className="p-4 bg-slate-50 rounded-lg hover:bg-slate-100 transition-all">
          <p className="font-semibold text-blue-600">Resume Summary</p>
          <p className="text-slate-600 text-sm">Tailor your resume summary for this specific role.</p>
        </li>
      </ul>
    </div>
  );
